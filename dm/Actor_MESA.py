...
class Actor(Agent):
    def __init__(self, unique_id, model, poles, currentState, desiredState, maxTime, error, history, criticalState, allActors, ioValues,
                 end_io_state):
   ...
   The poles will act again here, on the MST. The order in which these will act depends on the weights of the poles.
    ...
    def step(self): # a must-have function for mesa
        if self.unique_id==0:
            return
        original_resource=self.currentState.resources.copy()
        self.moves_made=self.makeDecision()
        
        afterDecision_resource=self.currentState.resources
        track_cost={}
        for key, value in original_resource.items():
            track_cost[key]=afterDecision_resource[key]-original_resource[key]
        for key,value in track_cost.items():
            [e for e in self.model.schedule.agents if e.unique_id==0][0].currentState.resources[key]= [e for e in self.model.schedule.agents if e.unique_id==0][0].currentState.resources[key]-value